...
test: manifests generate gowork fmt vet envtest ginkgo ## Run tests.
	OPERATOR_TEMPLATE=./templates KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" ginkgo $(GINKGO_ARGS)  ./... -coverprofile cover.out
